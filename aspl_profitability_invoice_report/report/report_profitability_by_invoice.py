...
class ProfitabilityReportPdf(models.AbstractModel):
    ...
    def _sql_query(self,obj):
        ...
        SQL = """SELECT am.id as move_id,am.name as number, am.invoice_date AS date,rp.name AS partner,am.move_type AS move_type,
                        ...
                        AND am.invoice_date >= '%s'
                        AND am.state = 'posted'
                        GROUP BY am.id,am.name,am.invoice_date,am.move_type,rp.name,ct.name,ct.id,pc.id,pp.id,aml.price_unit,aml.quantity""" % (" (%s) " % ','.join(map(str, sale_team_ids))," (%s) " % ','.join(map(str, partner_ids)),
        ...
        for data in result:
            ...
            cogs = -(product.standard_price * data.get('quantity')) if data.get('move_type') in ['out_refund', 'in_refund'] else product.standard_price * data.get('quantity')
                    ...
                    if custom_team.get('team_id') == each_team.get('team_id') and custom_team.get(
                        ...
                        custom_team.update(
                            {'sales': sale,
                             ...
                             'profitability': ((sale - cog) / sale) * 100})
                        count = count + 1
                    if custom_team.get('team_id') == each_team.get('team_id') and custom_team.get('category') == each_team.get('category') and custom_team.get('move_id') == each_team.get('move_id') and obj.report_view == "Detail":
                        sale = custom_team.get('sales') + each_team.get('sales')
                        cog = custom_team.get('cogs') + each_team.get('cogs')
                        custom_team.update(
                            {'sales': sale,
